<div class="clear"></div>
<footer>
  	<div class="menuFooter">
    	<nav id="boxMenuFooter">
        	<ul>
            	<li><a href="<?php echo get_option('urlsite2'); ?>/sobre-rain-bird.php">Sobre a Rain Bird</a></li>
                <li><a href="<?php echo get_option('urlsite2'); ?>/uso-inteligente-agua.php">O Uso Inteligente da Água</a></li>
                <li><a href="<?php echo get_option('urlsite2'); ?>/politica-privacidade.php">Política de Privacidade</a></li>
                <li><a href="<?php echo get_option('urlsite2'); ?>/termos-uso.php">Termos de Uso</a></li>
                <li><a href="<?php echo get_option('urlsite2'); ?>/contatos.php">Contatos</a></li>
            </ul>
        </nav>
    </div>
    <div class="linkOutros">
    	<a href="http://rainbird.com/worldwide/index.htm" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/images/bt-rain-bird-outros-paises.jpg" width="147" height="27" alt=""/></a>
    </div>
  </footer>
  <div class="clear"></div>
</div>
<div class="boxRedesSociais">
	<div><img src="<?php echo get_template_directory_uri(); ?>/images/redes-sociais.png" alt="" width="610" height="34" usemap="#MapRedes"/>
      <map name="MapRedes">
        <area shape="rect" coords="2,2,93,29" href="http://www.facebook.com/RainBirdCorp">
        <area shape="rect" coords="115,0,252,28" href="http://www.youtube.com/RainBirdCorp">
        <area shape="rect" coords="272,2,432,28" href="http://twitter.com/RainBirdCorp">
        <area shape="rect" coords="449,2,606,28" href="http://www.linkedin.com/company/rain-bird?trk=fc_badge">
      </map>
	</div>
    <p>Copyright &copy; 1996-2016 Rain Bird Brasil</p>
</div>

<?php wp_footer(); ?>
</body>
</html>