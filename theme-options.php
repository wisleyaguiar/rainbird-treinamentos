...
function theme_options_add_page() {
	add_theme_page( __( 'Opções do Site', 'sampletheme' ), __( 'Opções do Site', 'sampletheme' ), 'edit_theme_options', 'theme_options', 'theme_options_do_page' );
}
...
function theme_options_do_page() {
	...
	?>
	<div class="wrap">
		<?php screen_icon(); echo "<h2>" . get_current_theme() . __( ' :: Opções do Site', 'sampletheme' ) . "</h2>"; ?>

		<?php if ( false !== $_REQUEST['settings-updated'] ) : ?>
		<div class="updated fade"><p><strong><?php _e( 'Opções salvas com sucesso!', 'sampletheme' ); ?></strong></p></div>
		<?php endif; ?>
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				 ...
				 */
				?>
				<tr valign="top"><th scope="row"><?php _e( 'URL Site', 'sampletheme' ); ?></th>
					<td>
						<input id="sample_theme_options[urlsite2]" class="regular-text" type="text" name="sample_theme_options[urlsite2]" value="<?php esc_attr_e( $options['urlsite2'] ); ?>" />
						<label class="description" for="sample_theme_options[urlsite2]"><?php _e( 'URL do Site', 'sampletheme' ); ?></label>
					</td>
				</tr>
